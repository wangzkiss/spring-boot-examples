...
public class SingleSqlParserFactory{
	...
	{
		if(contains(sql,"(insert overwrite table)(.+)(select)(.+)(from)(.+)") || contains(sql,"(insert overwrite table)(.+)(partition)") || contains(sql,"(insert into)(.+)(as select)(.+)(from)(.+)")||contains(sql,"(insert into)(.+)(select)(.+)(from)(.+)") )
		{
		...
		}else if(contains(sql,"(create table)(.+)")||contains(sql,"(create external table)(.+)"))
		{
			return new CreateSqlParser(sql);
		}else if(contains(sql,"(alter table)(.+)"))
		{
			return new AlterSqlParser(sql);
		}
