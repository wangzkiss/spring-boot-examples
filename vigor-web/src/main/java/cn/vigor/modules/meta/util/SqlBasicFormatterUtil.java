...
public class SqlBasicFormatterUtil {
	...
	private static class FormatProcess {
		...
		public String perform() {
			...
			while (this.tokens.hasMoreTokens()) {
				...
				if ((this.afterByOrSetOrFromOrSelect) && (",".equals(this.token))) {
					...
					closeParen();
				} else if (SqlBasicFormatterUtil.BEGIN_CLAUSES.contains(this.lcToken)) {
					beginNewClause();
				} else if (SqlBasicFormatterUtil.END_CLAUSES.contains(this.lcToken)) {
					endNewClause();
				} else if ("select".equals(this.lcToken)) {
					select();
				} else if (SqlBasicFormatterUtil.DML.contains(this.lcToken)) {
				...
				} else if (SqlBasicFormatterUtil.LOGICAL.contains(this.lcToken)) {
					logical();
		...
		private void updateOrInsertOrDelete() {
			...
			if ("update".equals(this.lcToken)) {
				//TODO 如果是update语句,如下语句是需要的,现在考虑到查询字段存在关键词“update”的情况,顾先忽略update语句
//				newline();
			}
		...
		private static boolean isFunctionName(String tok) {
			...
			return (isIdentifier) && (!SqlBasicFormatterUtil.LOGICAL.contains(tok))
					&& (!SqlBasicFormatterUtil.END_CLAUSES.contains(tok))
					&& (!SqlBasicFormatterUtil.QUANTIFIERS.contains(tok)) && (!SqlBasicFormatterUtil.DML.contains(tok))
					&& (!SqlBasicFormatterUtil.MISC.contains(tok));
		}
