...
$(function () {
    var iframeLen=$(".J_mainContent").find("iframe");
    if(iframeLen.length==1&&iframeLen.data('id')=='/business-analysis/a/home'){
         var loading = layer.load(0, {shade: [1,'#fafafa']});
         setTimeout(function(){
             layer.close(loading);
         },600)
    }   
...
function scrollToTab(element) {
  ...
  // 解决选择过多后不显示对应选项卡
  if(scrollVal > 0){
    scrollVal = marginLeftVal - scrollVal
  }
...
// 关闭工作流页面，跳转到工作流任务
function closeMyselfTabs(){
      $('.page-tabs-content').children("[data-id]").not(":first").each(function () {
        if($(this)[0].innerText == '工作流设计'){
          $('.J_iframe[data-id="' + $(this).data('id') + '"]').remove();
          $(this).remove();
        }
        if($(this)[0].innerText == '工作流任务'){
          $('.J_iframe[data-id="' + $(this).data('id') + '"]').show();
          $(this).addClass("active");
        }
      });
  ...
  if (!$(this).hasClass('active')) {
    ...
    $("#side-menu .J_menuItem").each(function() {
      ...
      if ($this.attr("href") == currentId) {
        $('.J_menuItem').removeClass("active");
        $this.addClass("active").parents("ul").collapse("show").parents("li").addClass("active").siblings().removeClass("active").children("ul.in").collapse("hide");
      }
    })
...
//    });
    ...
    $("#side-menu .J_menuItem").each(function() {
      var $this = $(this);
      if (compareUrl($this.attr("href"), dataUrl)) {
        $('.J_menuItem').removeClass("active");
        $this.addClass("active").parents("ul").collapse("show").parents("li").addClass("active").siblings().removeClass("active").children("ul.in").collapse("hide");
      }
    });
    ...
    if(!isNew){
		    top.$('.J_menuTab').each(function () {
		        if (compareUrl(top.$(this).data('id'), dataUrl)) {// 选项卡已存在，激活。
		            if (!top.$(this).hasClass('active')) {
		                ...
		                top.$('.J_mainContent .J_iframe').each(function () {
		                    if (compareUrl(top.$(this).data('id'), dataUrl)) {
		                    	top.$(this).attr({'src': dataUrl, 'data-id': dataUrl}).show().siblings('.J_iframe').hide();
		                        return false;
		                    }
		                });
		            }
