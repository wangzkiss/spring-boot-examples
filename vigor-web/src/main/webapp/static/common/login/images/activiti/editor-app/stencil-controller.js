    ...
    .controller('StencilController', ['$rootScope', '$scope', '$http', '$modal', '$timeout', function ($rootScope, $scope, $http, $modal, $timeout) {
        ...
        $scope.editorFactory.promise.then(function () {
            ...
            $http({method: 'GET', url: KISBPM.URL.getStencilSet()}).success(function (data, status, headers, config) {
            	...
            	var quickMenuDefinition = ['ServiceTask', 'EndNoneEvent', 'ExclusiveGateway', 
            ...
            $scope.editor.registerOnEvent(ORYX.CONFIG.EVENT_SELECTION_CHANGED, function (event) {
                ...
                if (shapes && shapes.length == 1) {
                    ...
                    // 只有 ServiceTask 才显示查询和删除 2017/3/15 litiaxnin
        			if (morphShapes && morphShapes.length > 0 && morphShapes[0].id == "ServiceTask")
        			{
            ...
            $scope.selectTask = function() {
                ...
                $scope.safeApply(function () {
                    ...
                    if (shapes && shapes.length == 1)
                    {
                        ...
                        // property.value
                        var props = $rootScope.currentSelectedShape.properties;

                        // set task name
                        window.setTaskName = (function(){
                            return function(newVal){
                                var oldVal = props['oryx-name'];
                                props['oryx-name'] = newVal;
                                //
                                if(oldVal){
                                    jQuery('span:contains("' + oldVal + '"), ' +
                                        'input:contains("' + oldVal + '")').each(function(){
                                        var tag = jQuery(this);
                                        if(tag.is(':input')){
                                            if(oldVal !== tag.val()) return;
                                            tag.val(newVal);
                                        }else{
                                            if(oldVal !== tag.html()) return;
                                            tag.html(newVal);
                                        };
                                    });
                                };
                        ...
                        // get task name
                        window.getTaskName = (function(){
                            return function(taskName){
                                return props['oryx-name'];
                            };
                        }(props));
                        ...
                        var showSelectShapeDialog = function()
                        {
                            ...
                            $modal({
                                backdrop: false,
                                keyboard: true,
                                template: 'editor-app/popups/select-task.html'//?version=' + Date.now()
                            });
                        };
