...
<input type="text" id="${name}" name="${name}" maxlength="5" class="txt required" style="font-weight:bold;text-align:center;height:38px;padding:0;width:90px;${inputCssStyle}"/>
...
<a href="javascript:" onclick="$('.${name}').attr('src','${pageContext.request.contextPath}/servlet/validateCodeServlet?'+new Date().getTime());" class="mid ${name}Refresh" style="${buttonCssStyle}margin-left: 10px;margin-top:4px;">看不清</a>