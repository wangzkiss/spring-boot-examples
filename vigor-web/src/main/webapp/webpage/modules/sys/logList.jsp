...
<head>
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
	       ...
	       $("#contentTable tbody td:nth-child(6)").each(function(i){
...
<div class="ibox-title">
	...
	<div class="row">
	<div class="col-sm-12">
	<form:form id="searchForm" action="${ctx}/sys/log/" method="post"  class="form-inline form-box">
		...
		<div class="form-group">
			...
			<label>&emsp;至&emsp;</label><input id="endDate" name="endDate" type="text" maxlength="20" class=" laydate-icon form-control layer-date input-sm"
		...
		<div class="btn-form-box">
			<button style=""  class="btn btn-success btn-rounded btn-sm filter" onclick="search()" ><i class="fa fa-search"></i> 查询</button>
			<a href="javascript:"  class="btn btn-success btn-rounded btn-sm btn-outline filterReset" onclick="reset()" ><i class="fa fa-refresh"></i> 重置</a>
		</div>
	</form:form>
	</div>
	</div>
	...
	<div class="col-sm-12">
		<div class="pull-left">
			<shiro:hasPermission name="sys:log:del">
				...
				<button class="btn btn-white btn-sm toolbar "  onclick="confirmx('确认要清空日志吗？','${ctx}/sys/log/empty')"  title="清空"><i class="fa fa-trash"></i> 清空</button>
			</shiro:hasPermission>
	...
	<table id="contentTable" class="table table-bordered table-hover table-condensed dataTables-example dataTable table-fixed table-source table-store-cd table-store-rzlb table-checkbox">
		<colgroup>
			<col style="width:3%">
			<col style="width:15%">
			<col style="width:10%">
		...
		<c:forEach items="${page.list}" var="log">
			<tr>
				...
				<td class="viewBig">${log.title}</td>
				<td style="text-align: center;">${log.createBy.name}</td>
				<td>${log.createBy.company.name}</td>
				<td>${log.createBy.office.name}</td>
				<td class="viewBig"><strong>${log.requestUri}</strong></td>
				<td style="text-align: center;">${log.method}</td>
				<td style="text-align: center;">${log.remoteAddr}</td>
