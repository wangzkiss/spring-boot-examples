...
<body class="form-padding">
	<form:form id="inputForm" modelAttribute="role" autocomplete="off" action="${ctx}/sys/role/save" method="post">
		<form:hidden path="id"/>
		...
		<table class="table table-bordered  table-condensed dataTables-example dataTable no-footer table-inform">
		   ...
		   <tbody>
		      	<tr>
		         	<td>归属机构：</td>
		         	<td>
		         		<sys:treeselect id="office" name="office.id" value="${role.office.id}" labelName="office.name" labelValue="${role.office.name}" title="机构" url="/sys/office/treeData" cssClass="form-control input-sm required"/>
					</td>
			         <td><i class="star">*</i>角色名称：</td>
			         <td>
		      	...
		      	<tr>
		        	...
		        	<td>
		         	...
		         	<td>
						...
						<form:select path="roleType" class="form-control input-sm ">
						...
						<span class="help-inline" title="activiti有3种预定义的组类型：security-role、assignment、user 如果使用Activiti Explorer，需要security-role才能看到manage页签，需要assignment才能claim任务">工作流组用户组类型（任务分配：assignment、管理角色：security-role、普通角色：user）</span>
					</td>
		      	</tr>
		      	<tr>
		        	<td>是否系统数据：</td>
		        	<td>
		        		<form:select path="sysData" class="form-control  input-sm">
		      	...
		      	<tr>
		         <td>数据范围：</td>
		         <td><form:select path="dataScope" class="form-control input-sm">
